...
public class A7 {
	...
	final static int USE_PROVIDED = 1;
	final static int USE_YOURS = 3;
//	final static int USE = USE_YOURS;
		...
		for (int numThreads = minThreads; numThreads <= maxThreads; numThreads++) {
			...
			for (int k = 0; k < tries; k++) {
				...
				for (int m = 0; m < numThreads; m++) {
					...
					switch (USE) {
					case USE_PROVIDED:
						runningThreads.add(new ParallelWorkOriginal(m));
						break;
					case USE_YOURS:
						runningThreads.add(new ParallelWork(m));
						break;
					default:
						System.err.println("Unknown USE set in code so cannot run.  Exit with code " + USE_FAILED);
						System.exit(USE_FAILED);
				...
				switch (USE) {
				case USE_PROVIDED:
					ParallelWorkOriginal.setup(numThreads, curSeed);
					break;
				case USE_YOURS:
					ParallelWork.setup(numThreads, curSeed);
					break;
				default:
					System.err.println("Unknown USE set in code so cannot run.  Exit with code " + USE_FAILED);
					System.exit(USE_FAILED);
				...
				switch (USE) {
				case USE_PROVIDED:
					sumResult = ParallelWorkOriginal.returnSum();
					break;
				case USE_YOURS:
					sumResult = ParallelWork.returnSum();
					break;
				default:
					System.err.println("Unknown USE set in code so cannot run.  Exit with code " + USE_FAILED);
					System.exit(USE_FAILED);
